...
(function (d3) {
  ...
  function init () {
    ...
    d3.csv('./games.csv', d3.autoType)
      ...
      .catch(function (error) {
        ...
        showErrorMessage(
          'Failed to load chess games data. Please try again later.'
        )
      })
  }
  ...
  /**
   * @param root0
   * @param root0.buttonId
   * @param root0.menuId
   * @param root0.wrapperId
   * @param root0.options
   * @param root0.valueLabels
   */
  ...
  function applyFilters (data) {
    return data.filter((d) => {
      ...
      if (
        ...
        (white_rating < min || white_rating > max)
      ) { return false }
      if (
        colors.includes('black') &&
        (black_rating < min || black_rating > max)
      ) { return false }
  ...
  function updateFilters () {
    ...
    d3.selectAll('#checkbox-options input[type="checkbox"]:checked').each(
      function () {
        if (this.value !== 'all') {
    ...
    // filter for color
    state.filters.color = []
    d3.selectAll('#color-dropdown-menu input[type="checkbox"]:checked').each(
      function () {
    ...
    d3.selectAll(
      '#game-type-dropdown-menu input[type="checkbox"]:checked'
    ).each(function () {
    ...
    d3.selectAll(
      '#time-control-dropdown-menu input[type="checkbox"]:checked'
    ).each(function () {
