...
export class LineChartVisualization extends VisualizationBase {
  ...
  createTooltipContent(opening, dataPoint) {
    return `
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <tr>
          <td style="padding: 4px 0; border-bottom: 1px solid #eee;">Victoire des blancs : </td>
          <td style="padding: 4px 0; border-bottom: 1px solid #eee;">${dataPoint.whiteWinPct.toFixed(
            2
          )}%</td>
        </tr>
        <tr>
          <td style="padding: 4px 0; border-bottom: 1px solid #eee;">Victoire des noirs : </td>
          <td style="padding: 4px 0; border-bottom: 1px solid #eee;">${dataPoint.blackWinPct.toFixed(
            2
          )}%</td>
        </tr>
        <tr>
          <td style="padding: 4px 0;">Égalité : </td>
          <td style="padding: 4px 0;">${dataPoint.drawPct.toFixed(2)}%</td>
        </tr>
        <tr>
          <td style="padding: 4px 0; border-top: 1px solid #eee;">Total parties : </td>
          <td style="padding: 4px 0; border-top: 1px solid #eee;">${
            dataPoint.total
          }</td>
        </tr>
      </table>
  ...
  createLineChartLegend(data, colorScale) {
    ...
    if (data.length * itemHeight > legendMaxHeight - 10) {
      const foreignObject = legendGroup
      ...
      data.forEach((d, i) => {
        ...
        <svg width="24" height="${itemHeight}" style="flex-shrink:0;">
          <line x1="0" y1="${itemHeight / 2}" x2="20" y2="${
          itemHeight / 2
        }" stroke="${colorScale(
          d.name
        )}" stroke-width="3" stroke-linecap="round"/>
          <circle cx="10" cy="${itemHeight / 2}" r="4" fill="${colorScale(
          d.name
        )}" stroke="white" stroke-width="1"/>
        </svg>
        <span style="font-size:12px; color:#333; margin-left:4px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:${
          this.options.legendWidth - 40
        }px;">${this.truncateName(d.name, 20)}</span>
      `);
      });
    } else {
